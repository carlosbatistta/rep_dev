...
class ListAddressedStockController {
    handle: RequestHandler = async (req: Request, res: Response) => {
        try {
            const { product_code, storage_code, reserve_quantity, transfer_quantity, address_code } = req.body;
            ...
            const addressed_stock = await listAddressedStockService.execute({
                product_code, storage_code, reserve_quantity, transfer_quantity, address_code
            });
