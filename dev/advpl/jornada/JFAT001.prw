...
ConOut( "JFAT001 - Inùcio " + Dtoc( Date() ) + " " + Time() )
...
aAdd(aNatFil, {"01010101001","1211003002"}) //Trasnf. mercadoria filial Custùdia: 1211003002 
aAdd(aNatFil, {"01010101002","1211003003"}) //Trasnf. mercadoria filial Afogados: 1211003003
aAdd(aNatFil, {"01010101003","1211003004"}) //Trasnf. mercadoria filial Sertùnia: 1211003004
...
//Local nRecnoSC6
Local nVlrUnit
//Ita - 21/08/2023 - Local cTabPrc     := "003"             //tabela de preùo definida
Local cNaturez    := "1211003001"      //cùdigo da natureza definido
Local aLastQuery   
Local cLogErro    := ""
Local nOpc        := 0
Local nCount      := 0
...
Local nVlrDesc
Local nVlrUnit
Private cTabPrc     := "003"             //tabela de preùo definida
Private _CGC := Alltrim( SM0->M0_CGC )

//pedido normal
//vendedor 1 informado
//tabela de preùo nùo informada
//natureza nùo informado
//pedido nùo faturado
...
If !Eof()
   ...
   BEGIN TRANSACTION
      ...
      Do While !Eof()
         ...
         If nPsNat > 0
            cNaturez := aNatFil[nPsNat,2] //Obtùm a natureza correspondente a filial
         EndIf
         ...
         //aAdd( aCabec, { "C5_VEND5"   , cVnd5      , Nil } ) //Vendedor 5 - (Todos os pedidos devem ter a mesma quantidade de vendedores, registrados os mesmos cùdigos, e nos mesmos campos)
...
- Condiùùo Pagamento (C5_CONDPAG) [desde que seja diferente do Tipo 9, conforme faq em anexo]
- Vendedores (C5_VEND'x') (Todos os pedidos devem ter a mesma quantidade de vendedores, registrados os mesmos cùdigos, e nos mesmos campos)
- Tipo Frete (C5_TPFRETE)
- Transportadora (C5_TRANSP)
- ISS Incluso (C5_INCISS)
- Recolhe ISS (C5_RECISS)
- Fornecedor ISS (C5_FORNISS)
- Presenùa Com (C5_INDPRES)
         */
...
//Excluùdo bloco abaixo [Mauro Nagata, wwww.lurin.com.br, 20220502]
//busca o preùo do produto na tabela de preùo
Static Function BuscaPrc( cTabela, cCodProd )        
Local aLastQuery  := {} 
Local cLastQuery  := ""
Local cAlias 	   := GetNextAlias() 
//Ita - 17/07/2023 Local nVlrUnit    := 999999999      //caso nùo localize o valor do produto na tabela de preùo
Local nVlrUnit    := 1      //Ita - 17/07/2023 - caso nùo localize o valor do produto na tabela de preùo
...
//se encontrar o produto na tabela de preùo 
If !Eof()
   aLastQuery    := GetLastQuery()
   cLastQuery    := aLastQuery[2] 
...
/// Ita - 17/07/2023
///       Funùùo fGetUNFE
///       Obttùm valor da ùltima NF de Entrada.
///////////////////////////////////////////////
...
Static Function fUpdOrcs
   ...
   If DbSeek( xFilial( "SL2" ) + cNumOrc )
      ...
      Do While !Eof() .And. SL2->L2_FILIAL = xFilial( "SL2" ) .And. SL2->L2_NUM == cNumOrc
         ...
         //ConOut( "Gravaùùo Item Orùamento SL2 - Orùamento: " + cNumOrc + " Pedido de venda: " + SC5->C5_NUM + " Produto: " + SC6->C6_PRODUTO + " Preùo: "+ Str( SC6->C6_PRCVEN, 14, 2 ) )
      ...
      DbSelectArea( "SL1" )      //orùamento
      DbSetOrder(1)
      If DbSeek( xFilial( "SL1" ) + cNumOrc )
         
         ConOut( "Achou SL1 - Orùamento: " + xFilial( "SL1" )+"|"+cNumOrc )
         ...
         ConOut( "Gravaùùo Orùamento SL1 - Orùamento: " + cNumOrc )


         //ConOut( "Gravaùùo Orùamento SL1 - Orùamento: " + cNumOrc + " Pedido de venda: " + SC5->C5_NUM + " Produto: " + SC6->C6_PRODUTO + " Total C/Desc: "+ Str(  nTotCDesc, 14, 2 ) + " Total S/Desc: "+ Str(  nTotSDesc, 14, 2 ) )

      EndIf

      DbSelectArea( "SL4" )   //condiùùo negociada
      DbSetOrder(1)
      If DbSeek( xFilial( "SL4" ) + cNumOrc )

         ConOut( "Achou SL4 - Orùamento: " + xFilial( "SL4" ) +"|"+ cNumOrc )

         RecLock( "SL4", .F. )
         SL4->L4_VALOR     := nTotCDesc
         SL4->( MsUnLock() )

         ConOut( "Gravaùùo Orùamento SL4 - Orùamento: " + cNumOrc )

         //ConOut( "Gravaùùo Orùamento SL4 - Orùamento: " + cNumOrc + " Pedido de venda: " + SC5->C5_NUM + " Produto: " + SC6->C6_PRODUTO + " Total C/Desc: "+ Str(  nTotCDesc, 14, 2 ) )
      EndIf
