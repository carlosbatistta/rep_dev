...
//! Descrição: Gravação da Nota de Entrada na  Unidade de Faturamento
//! Rotina automática: MATA103 - DOCUMENTO TIPO NORMAL
//!####################################################################
// DOCUMENTO TIPO NORMAL VINCULADO AO PEDIDO DE COMPRAS
User Function DLGENTFAT()
	@ 0,0 to 300,300 Dialog oDlg Title "3-Gera NF Entrada na Unid Faturamento"
...
Static Function GDENTUNIFAT()
    ...
    cQtdeNF += " F2.F2_XNFENT1 = 'N'" //! verifica se ainda não foi geada a nota de entrada
    cQtdeNF := ChangeQuery(cQtdeNF)
    AliasQtde := CriaTrab(,.F.)

    DbUseArea(.T., "TOPCONN", TCGenQry(,,cQtdeNF), AliasQtde, .F., .T.)

    // SELECT COUNT(*) AS NOTAS, F2.F2_DOC FROM SF2010 AS F2 WHERE  F2.D_E_L_E_T_ <> '*' AND F2.F2_XCONSOL = 'S' AND F2.F2_XNFENT1 = 'N' GROUP BY  F2.F2_DOC
    ...
    For nX := 1 To (AliasQtde)->NOTAS
        ...
        If !(AliasF2)->(Eof())
            ...
            cFilAnt := _cUnidFat //! seta a filial de faturamento
            ...
            cQrySZX += " RTRIM(LTRIM(ZX.ZX_CCCON)) AS ZXCCCON"
            ...
            If !(AliasSZX)->(Eof())
                ...
                cZXCCCON   := AllTrim((AliasSZX)->ZXCCCON)
            EndIf
            ...
            cQryD2 += " D2.D2_FILIAL AS 'FILIAL', D2.D2_DOC AS 'D2DOC', D2.D2_SERIE AS 'D2SERIE', D2.D2_CLIENTE AS 'D2CLIENTE', D2.D2_LOJA AS 'D2LOJA',"
            ...
            While !((AliasD2))->(Eof())
                ...
                //* verifica o grupo tributário para definição do TES
                dbSelectArea("SB1")
                dbSetOrder(1)
                dbSeek(xFilial("SB1") + (AliasD2)->D2COD)
                _cB1GRTRIB := Alltrim(SB1->B1_GRTRIB)
                _cTES := "016"
                If _cB1GRTRIB $ "000001/000002/000003"
                    _cTES := "022"
                EndIf
                If _cB1GRTRIB $ "000004"
                    _cTES := "026"
                EndIf
                //* ---------------------------------------------
            ...
            If !lMsErroAuto
                Alert("Nota Fiscal de Entrada: " + _cDoc + ", entrou na Unidade de Faturamento")
                cQUpdF2 :=""
                cQUpdF2 +="UPDATE " + RetSqlName("SF2") + " SET F2_XNFENT1 ='S'"
